...
function checkOnKare() {
  ...
  try {
    const rankCounts =
        {};  // Объект для подсчета количества каждого номинала карт
    selectedCards = selectedCards.sort();
    // Подсчет количества каждого номинала в выбранных картах
    for (const card of selectedCards) {
      const [, rank] =
          card;  // Нам нужен только номинал карты, игнорируем масть
    ...
    if (kareRank) {
      messageElement.textContent = `Вы собрали Каре с картами ${kareRank}!`;

      const messageContainer = document.getElementById('messageContainer');
      messageContainer.appendChild(
          messageElement);  // Добавляем сообщение в контейнер
    } else {
      throw new Error('В выбранных картах нет Каре. НЕТ ИГРЫ');
    }
  } catch (error) {
    ...
    console.log(
        'Проверка завершена.');  // Выполняется всегда после try или catch
  }
}
