...
// Функция загрузки деталей ваучера в форму редактирования
function loadVoucherDetails() {
  const voucherId = $('#editVoucherSelect').val();
  if (voucherId) {
    $.ajax({
      url: `/api/vouchers/${voucherId}`,
      type: 'GET',
      success: function(voucher) {
        $('#editCountry').val(voucher.country);
        $('#editDepartureDate').val(voucher.departureDate);
        $('#editDuration').val(voucher.duration);
        $('#editStars').val(voucher.stars);
        $('#editHotelName').val(voucher.hotelName.join(', '));
      },
...
// Функция редактирования ваучера
function editVoucher() {
  ...
  const updatedVoucher = {
    ...
    stars: $('#editStars').val(),
    hotelName: $('#editHotelName').val().split(',').map(name => name.trim())
  };
  ...
  if (voucherId) {
    $.ajax({
      ...
      success: function() {
        alert('Данные ваучера успешно обновлены.');
        loadEditVouchers();
      },
      error: function(error) {
    ...
    alert('Выберите ваучер для редактирования.');
  }
}
...
function deleteVoucher() {
  ...
  if (voucherId) {
    ...
    alert('Выберите ваучер для удаления.');
  }
}
  // Функция загрузки ваучеров для редактирования
  function loadEditVouchers() {
    $('#editVoucherSelect').empty();
    $.ajax({
      url: '/api/vouchers',
      type: 'GET',
      success: function(vouchers) {
        vouchers.forEach(voucher => {
          const option = `<option value="${voucher._id}">${voucher.country} ${voucher.departureDate}</option>`;
          $('#editVoucherSelect').append(option);
        });
      },
      error: function(error) {
